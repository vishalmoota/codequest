...
const hasHtmlDocumentShell = (source = '') => /<html[\s>]/i.test(source) && /<body[\s>]/i.test(source);

const wrapHtmlDocument = (htmlFragment = '', projectId = 'codequest-project') => {
  if (!htmlFragment.trim()) return '';
  if (hasHtmlDocumentShell(htmlFragment)) return htmlFragment;
...
const buildCombinedProjectHtml = (stepCodes = {}, sortedStepIndexes = [], projectId = 'codequest-project') => {
  const combinedFragments = sortedStepIndexes
    .map((stepIndex) => stepCodes[String(stepIndex)] || '')
    .map((snippet) => snippet.trim())
    .filter(Boolean)
    .join('\n\n');

  if (!combinedFragments) return '';
  return wrapHtmlDocument(combinedFragments, projectId);
};
...
router.get('/:projectId/download', auth, async (req, res) => {
  ...
  try {
    ...
    const fallbackFinalCode = stepCodes[String(latestStepIndex)] || Object.values(stepCodes).pop() || '';
    const combinedProjectHtml = buildCombinedProjectHtml(stepCodes, sortedStepIndexes, req.params.projectId);
    const finalCode = combinedProjectHtml || wrapHtmlDocument(fallbackFinalCode, req.params.projectId);

    fs.writeFileSync(path.join(projectDir, 'index.html'), finalCode, 'utf8');
    fs.writeFileSync(path.join(projectDir, 'styles.css'), '/* Optional: move your CSS here. */\n', 'utf8');
    fs.writeFileSync(path.join(projectDir, 'script.js'), '// Optional: move your JavaScript here.\n', 'utf8');
    fs.writeFileSync(
      path.join(projectDir, 'README.md'),
      `# ${req.params.projectId}\n\nGenerated by CodeQuest Project Tutorials.\n\n## Run locally\n\n1. Extract the ZIP file.\n2. Open index.html in your browser.\n3. (Optional) Move styles into styles.css and scripts into script.js.\n`,
      'utf8'
    );
